...
function validateName(userName) {
  ...
  if (regEx.test(userName) === false)
    return "The name you inserted is invalid.";
...
function validateLastName(lastName) {
  ...
  if (regEx.test(lastName) === false)
    return "The last name you inserted is invalid.";
...
function validateEmail(email) {
  ...
  if (email.length === 0) return "The email field cannot be empty.";
...
function deletePreviousErrors() {
  document
    .querySelectorAll(".existing-error")
    .forEach(function (existingError) {
